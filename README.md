...
SAC is a popular algorithm for continuous control tasks (although a discrete version does exist). It notably different from other mainstream RL algorithms because it formulates the RL objectivate differently. Normally the objective is formulated as maximizing the expected return sum(r). However, SAC utilizes a maximum entropy framework which means it tried to maximize sum(r + entropy). This (supposedly) results in more robost policies. This algorithm is SotA on most benchmarks; however, it is one of the more difficult algorithms to implement. There are also several versions, I implement the most recent one (to the best of my knowledge). The original paper had two Q approximators and a V approximator and required the temperature parameter to be manually defined. The more recent version drops the value network, adds target networks for the Q networks and automatically adjusts the temperature. 
